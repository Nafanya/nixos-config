...
in
{
  ...
  config = mkIf cfg.enable {
    assertions = [ (lib.hm.assertions.assertPlatform "services.spotifyd" pkgs lib.platforms.linux) ];
    ...
    systemd.user.services.spotifyd = {
      ...
      Service = {
        ExecStart = "${cfg.package}/bin/spotifyd --no-daemon --config-path ${cfg.configFile}";
        Restart = "always";
        RestartSec = 12;
      };
    };
